...
export default {
  ...
  computed: {
    ...
    // },
    // 简写
    ...mapGetters(["categoryView", "goodsInfo", "spuSaleAttrList"]),
    // 图片列表
    imgList() {
  ...
  beforeMount() {
    // 一般在 beforeMount 中处理数据，处理完成后后期就可以直接使用
  ...
  methods: {
     ...
     */
    async addCart() {
      // 向后台发请求，向后台数据表中添加一条数据
      // 添加成功与否，决定要不要跳转到添加购物车成功页面
      let { goodsId, goodsNum } = this;
      // dispatch 相当于action函数调用表达式,值是action函数的返回值 Promise类型

      // 下面可能成功，可能返回失败的Promise
      // const res = await this.$store.dispatch("addOrUpdateCart", {
      //   goodsId,
      //   goodsNum,
      // });
      /**这种方式只有成功,需要判断成功结果的不同 */
      // if(res.code===200){
      // }else{
      // }
      ...
      try {
        ...
        // });
        alert("添加购物车成功!");
        // 需要传递到新页面：商品数量和商品详情信息 
        // 路由传参都是带过去的简单数据：数字、字符串，不会带对象
        // 只要两个路由之间传递的是复杂数据，全部考虑 存储手段
        // 简单数据：路由传参
        // 复杂数据：存储传参 
        // localStorage（永久存储）、sessionStorage(临时存储，浏览器关闭就没了)
        // setItem、getItem、removeItem、clear
        sessionStorage.setItem('GOODSINFO_KEY',JSON.stringify(this.goodsInfo))
